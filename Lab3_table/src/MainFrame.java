...
// манипулировать из разных мест
    ...
    private JMenuItem outputInformationMenuItem;
...
// и инициализировать текущей директорией
                    fileChooser.setCurrentDirectory(new File("C:\\Users\\User\\Desktop\\For Java.txt"));
                }
...
// Создать новое действие по выводу данных обо мне
        Action outputInformationAction = new AbstractAction("Об программе") {
            public void actionPerformed(ActionEvent event) {
                Icon authorPhoto = new ImageIcon(getClass().getResource("Author.png"));
                JOptionPane.showMessageDialog(null, "Автор задачи: Королёв Николай\n6 группа 2 курс",
                        "Информация об авторе", 3, authorPhoto);
            }
        };
        // Добавить действие в меню "Таблица"
         outputInformationMenuItem = referenceMenu.add(outputInformationAction);
        outputInformationMenuItem.setEnabled(true);
...
// На основе считанных данных создать новый экземпляр модели таблицы
                    ...
                    data = new GornerTableModel(from, to, step, MainFrame.this.koeff);
