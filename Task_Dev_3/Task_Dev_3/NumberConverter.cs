...
namespace Task_Dev_3
{
    public class NumberConverter
    {
        ...
        public string Converter()
        {
            ...
            if (newBase > 20 || newBase < 2 || number < 0)
            {
            ...
            while (number > 0)
            {
                ...
                foreach (var digit in toDigitMore10)
                {
                    ...
                    {
                        isInDictionary = true;
                        numberInNewBase.Append(digit.Value);
                    }
                }
                if (isInDictionary == false)
                    numberInNewBase.Append(number % newBase);
                number = (int)(number / newBase);
                isInDictionary = false;
            }

            char[] answ = (numberInNewBase.ToString()).ToCharArray();
            numberInNewBase.Clear();
            for (int i = answ.Length - 1; i >= 0; i--)
            {
                numberInNewBase.Append(answ[i]);
            }
            return numberInNewBase.ToString();
        }
    }
}
